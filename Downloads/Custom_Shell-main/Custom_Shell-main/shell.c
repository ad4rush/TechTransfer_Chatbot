...
int execute_date_d_y(const char *arg){
    int pid = fork();
    ...
    if (pid == -1) {
        ...
        char arg_copy[strlen(arg) + 1];
        strcpy(arg_copy, arg);
        char *args[] = {"date", "-d", "yesterday",arg_copy, NULL};
        execvp("./date",args);
        exit(1);
    }else {
        // Parent process
...
int execute_date_d_t(const char *arg){
    int pid = fork();
    ...
    if (pid == -1) {
        ...
        char arg_copy[strlen(arg) + 1];
        strcpy(arg_copy, arg);
        char *args[] = {"date", "-d", "tomorrow",arg_copy, NULL};
        execvp("./date",args);
        exit(1);
    }else {
        // Parent process
            ...
            if (command) {
                if (strcmp(command, "word") == 0) {
                    ...
                    if ((arg && strcmp(arg, "-d") == 0)) {
                        d = 1;
                        arg = strtok(NULL, " ");
                        if((arg && strcmp(arg, "\'yesterday\'") == 0)){

                            y=1;
                        }
                        else if((arg && strcmp(arg, "\'tomorrow\'") == 0)){
                            t=1;
                        }
                        arg = strtok(NULL, " ");
                        ...
                        if (arg) {
                            ...
                            if (extra_arg) {
                                ...
                                execute_date_d_y(arg);
                                continue;
                                }
                                else if(t){
                                execute_date_d_t(arg);
                                continue;
                                }
                                else {
                                    printf("Usage: date [-d] <yesterday/tomorrow> filename\n");
                                    continue;
                        }
